...
function homePage() {
  Promise.all([
    getMovies("https://api.themoviedb.org/3/movie/now_playing?api_key=821e6e287624c7921335f083519db105&language=en-US&page=1", "now-playing"),
...
async function predictions() {
  ...
  if (query) {
    ...
    if (searchCall) {
      const searchJSON = await searchCall.json();
      const searchArr = searchJSON.results;
      let results = [];
      let makeUp = "";
      for (let i = 0; i < 7; i++) {
      ...
      document.getElementById("myPredict").style.display = "block";
      document.getElementById("myPredict").innerHTML = makeUp;
    }
  }
}
